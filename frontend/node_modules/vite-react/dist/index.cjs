"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; } function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }// src/vite.ts
...
var handleCss = (config, env) => {
  ...
  _nullishCoalesce((_b = config.css.modules).generateScopedName, () => ( (_b.generateScopedName = env.command === "build" ? "[hash:base64]" : "[path][name]-[ext]___[local]")));
};
...
var handleHtml = (config, env) => {
  ...
  _nullishCoalesce((_a = config.html).minify, () => ( (_a.minify = env.command === "build")));
...
var handleMix = (config) => {
  _nullishCoalesce(config.base, () => ( (config.base = "./")));
  _nullishCoalesce(config.clearScreen, () => ( (config.clearScreen = false)));
};
...
var handleResolve = (config) => {
  ...
  if (aliasFromTsconfig) {
    config.plugins || (config.plugins = []);
    config.plugins.push(
      _vitetsconfigpaths2.default.call(void 0, aliasFromTsconfig === true ? void 0 : aliasFromTsconfig),
      _vitepluginrestart2.default.call(void 0, { restart: ["**/tsconfig.json"], glob: true })
    );
  }
};
...
var _vitepluginmkcert = require('vite-plugin-mkcert'); var _vitepluginmkcert2 = _interopRequireDefault(_vitepluginmkcert);
var _vitepluginqrcode = require('vite-plugin-qrcode');
var handleServer = (config) => {
  var _a;
  config.server || (config.server = {});
  config.preview || (config.preview = {});
  _nullishCoalesce((_a = config.server).open, () => ( (_a.open = true)));
  config.plugins || (config.plugins = []);
  if (config.server.https === true) {
    config.server.https = void 0;
    config.plugins.push(_vitepluginmkcert2.default.call(void 0, { source: "coding" }));
  }
